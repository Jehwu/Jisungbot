...
def get_max_fatigue(u):
    return 100 + (u.get("stat_health_level", 0) * 10)
...
# ---------------------------------------------------------
HEALTH_STAT_SPECS = {
    1: {"max_fatigue": 110, "cost": 50000},
    2: {"max_fatigue": 120, "cost": 100000},
    3: {"max_fatigue": 130, "cost": 180000},
    4: {"max_fatigue": 140, "cost": 280000},
    5: {"max_fatigue": 150, "cost": 400000},
    6: {"max_fatigue": 160, "cost": 550000},
    7: {"max_fatigue": 170, "cost": 720000},
    8: {"max_fatigue": 180, "cost": 920000},
    9: {"max_fatigue": 190, "cost": 1150000},
    10: {"max_fatigue": 200, "cost": 1400000}
}

TENACITY_STAT_SPECS = {
    1: {"rate": 5.0, "cost": 500000},
    2: {"rate": 10.0, "cost": 1100000},
    3: {"rate": 15.0, "cost": 1800000}
}

POOP_LEVEL_SPECS = {
    1: {"mult": 1.0, "cost": 0, "art_cost": 0},
    2: {"mult": 1.2, "cost": 50000, "art_cost": 1},
    3: {"mult": 1.44, "cost": 150000, "art_cost": 2},
    4: {"mult": 1.73, "cost": 400000, "art_cost": 3},
    5: {"mult": 2.07, "cost": 1000000, "art_cost": 5}
}
...
DANCE_LEVEL_REQS = [50, 150, 300, 500, 750]
...
def update_dance_level(u):
    dc = u.get("dance_count", 0)
    lvl = 0
    for i, req in enumerate(DANCE_LEVEL_REQS):
        if dc >= req:
            lvl = i + 1
    u["dance_level"] = lvl
    return lvl
...
def get_user_data(data, user_id):
    ...
    if uid not in data["users"]:
        ...
        u = data["users"][uid]
        u.setdefault("stat_health_level", 0)
        u.setdefault("stat_tenacity_level", 0)
        u.setdefault("poop_level", 1)
        u.setdefault("last_poop_time", 0)
        u.setdefault("poop_count_today", 0)
        ...
        u.setdefault("dance_count", 0)
        u.setdefault("dance_level", 0)
...
DUNGEON_DATA = {
    1: {"name": "1층: 동네 고가도로", "rec_lvl": 3, "boss": "🛵 딸배왕 박씨", "boss_speed": 15, "gold": 20000, "stones": 2, "protect": 0, "degrade_protect": 0},
    2: {"name": "2층: 수도권 외곽순환", "rec_lvl": 8, "boss": "🏎️ 야간 칼치기 폭주족", "boss_speed": 25, "gold": 50000, "stones": 0, "protect": 0, "degrade_protect": 1},
    3: {"name": "3층: 태백 레이스웨이", "rec_lvl": 13, "boss": "🏁 프로 레이싱팀 에이스", "boss_speed": 38, "gold": 100000, "stones": 5, "protect": 0, "degrade_protect": 0},
    4: {"name": "4층: 영암 F1 서킷", "rec_lvl": 18, "boss": "🏆 전직 F1 챔피언", "boss_speed": 55, "gold": 250000, "stones": 0, "protect": 1, "degrade_protect": 0},
    5: {"name": "5층: 뉘르부르크링", "rec_lvl": 23, "boss": "👻 뉘르의 유령 드라이버", "boss_speed": 75, "gold": 500000, "stones": 0, "protect": 0, "degrade_protect": 2},
    6: {"name": "6층: 아우토반 무제한", "rec_lvl": 28, "boss": "👑 속도의 신 [지성]", "boss_speed": 100, "gold": 1000000, "stones": 0, "protect": 2, "degrade_protect": 0}
}
...
async def update_artifact_and_fish_prices():
    data = load_data()
    art_base = {
        "똥먹방 비법서": (30000, 6000, 90000), "차은우지성 조각상": (50000, 10000, 150000),
...
class FishSellModal(discord.ui.Modal):
    def __init__(self, fish_item_key, user_id):
        super().__init__(title=f"🐟 {fish_item_key} 개별 매도")
        self.fish_item_key = fish_item_key
        self.user_id = user_id
        self.qty_input = discord.ui.TextInput(label="판매할 마릿수 입력", placeholder="예: 3", min_length=1, max_length=5)
        self.add_item(self.qty_input)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        data = load_data()
        u = get_user_data(data, self.user_id)
        curr_cnt = u["inventory"].get(self.fish_item_key, 0)

        if curr_cnt < qty:
            await interaction.response.send_message(f"❌ 보유 마릿수({curr_cnt}마리)보다 더 많은 수량을 입력하셨습니다.", ephemeral=True)
            return

        base_fish = self.fish_item_key.replace("🌀 혼돈 ", "").replace("🐘 거대 ", "")
        p_info = data["market"]["fish"].get(base_fish, {"price": 1000})
        base_price = p_info["price"] if isinstance(p_info, dict) else p_info

        mult = 1.0
        if "🌀 혼돈" in self.fish_item_key: mult = 3.0
        elif "🐘 거대" in self.fish_item_key: mult = 2.0

        unit_price = int(base_price * mult)
        total = unit_price * qty

        u["inventory"][self.fish_item_key] -= qty
        u["money"] += total
        save_data(data)

        new_embed = build_fish_bag_embed(interaction.user, u)
        new_view = FishBagView(self.user_id, u)
...
# --- 개편된 /똥싸기 3x3 타일 탐사 게임 View ---
class PoopTileButton(discord.ui.Button):
    def __init__(self, x, y, score_val, emoji, name):
        super().__init__(style=discord.ButtonStyle.secondary, label="❓", row=y)
        self.x = x
        self.y = y
        self.score_val = score_val
        self.tile_emoji = emoji
        self.tile_name = name

    async def callback(self, interaction: discord.Interaction):
        game_view: PoopGameView = self.view
        if interaction.user.id != game_view.user_id:
            await interaction.response.send_message("❌ 본인의 게임만 진행할 수 있습니다.", ephemeral=True)
            return

        self.disabled = True
        self.label = self.tile_name
        self.emoji = self.tile_emoji

        if self.score_val < 0:
            self.style = discord.ButtonStyle.danger
        elif self.score_val >= 3:
            self.style = discord.ButtonStyle.success
        else:
            self.style = discord.ButtonStyle.primary

        game_view.clicks += 1
        game_view.scores.append(self.score_val)
        game_view.history.append(f"• {game_view.clicks}번째 선택: {self.tile_emoji} **[{self.tile_name}]** ({self.score_val:+d}점)")

        if game_view.clicks < 3:
            await interaction.response.edit_message(embed=game_view.get_grid_embed(), view=game_view)
        else:
            for child in game_view.children:
                child.disabled = True

            total_score = sum(game_view.scores)
            data = load_data()
            u = get_user_data(data, interaction.user.id)

            level_spec = POOP_LEVEL_SPECS.get(game_view.poop_level, POOP_LEVEL_SPECS[1])
            mult = level_spec["mult"]

            # 3점 타일(황금똥)을 뽑았을 때 극적 보상
            has_gold_tile = 3 in game_view.scores

            score_str = f"• 총 점수: **{total_score}점** ({game_view.scores[0]:+d}점 / {game_view.scores[1]:+d}점 / {game_view.scores[2]:+d}점)"
            
            if has_gold_tile and total_score >= 6:
                res_title = "🌟 [전설의 럭키 황금똥!]"
                base_fatigue, base_money = 60, 50000
                img_file = "똥_황금똥.png"
                ...
                res_title = "✨ [매끈한 쾌변똥]"
                base_fatigue, base_money = 25, 6000
                img_file = "똥_쾌변똥.png"
                embed_color = 0x2ecc71
            elif total_score >= 1:
                res_title = "💩 [평범한 잔변똥]"
                base_fatigue, base_money = 10, 1500
                img_file = "똥_잔변똥.png"
                embed_color = 0x3498db
            else:
                res_title = "💥 [치질 & 혈변 대참사]"
                base_fatigue, base_money = -15, -5000
                img_file = "똥_치질.png"
                embed_color = 0xe74c3c

            if base_money > 0:
                final_fatigue = int(base_fatigue * mult)
                final_money = int(base_money * mult)
            else:
                final_fatigue = base_fatigue
                final_money = base_money

            max_f = get_max_fatigue(u)
            if final_fatigue > 0: u["fatigue"] = min(max_f, u["fatigue"] + final_fatigue)
            else: u["fatigue"] = max(0, u["fatigue"] + final_fatigue)

            u["money"] += final_money
            save_data(data)

            history_str = "\n".join(game_view.history)
            res_embed = discord.Embed(
                title=f"🚽 쾌변 탐사 결과 - {res_title}",
                description=(
                    f"**[ 오픈한 타일 목록 ]**\n{history_str}\n\n"
                    f"{score_str}\n"
                    f"• **레벨 보상 배율:** `x{mult:.2f}`\n"
                    f"• **피로도 변화:** `{final_fatigue:+d}` (현재: {u['fatigue']}/{max_f})\n"
                    f"• **획득/손실 금액:** `{final_money:+d}원` (잔액: {u['money']:,}원)"
                ),
                color=embed_color
            )
            res_embed.set_thumbnail(url=get_img_url(f"똥_{game_view.poop_level}단계.png"))
            res_embed.set_image(url=get_img_url(img_file))
            await interaction.response.edit_message(embed=res_embed, view=game_view)
...
class PoopGameView(discord.ui.View):
    def __init__(self, user_id, poop_level):
        ...
        # 9개 타일 덱 생성 (1x 황금, 2x 바나나, 3x 잔변, 3x 치질)
        tiles = [
            (3, "🎯", "황금똥"),
            (2, "🍌", "바나나똥"), (2, "🍌", "바나나똥"),
            (1, "💩", "일반똥"), (1, "💩", "일반똥"), (1, "💩", "일반똥"),
            (-2, "💥", "치질/혈변"), (-2, "💥", "치질/혈변"), (-2, "💥", "치질/혈변")
        ]
        ...
        for y in range(3):
            for x in range(3):
                val, emo, name = tiles[idx]
                self.add_item(PoopTileButton(x, y, val, emo, name))
                idx += 1

    def get_grid_embed(self):
        desc = (
            f"• 9개의 탐사 타일 중 **원하는 위치의 타일 3개**를 선택하세요!\n"
            f"• **현재 진행 상황:** `[{self.clicks}/3]`회 선택\n\n"
            "**[ 타일별 점수 분포 ]**\n"
            "🎯 황금똥(`+3점`) | 🍌 바나나똥(`+2점`) | 💩 일반똥(`+1점`) | 💥 치질(`-2점`)\n"
        )
        if self.history:
            desc += "\n**[ 열어본 타일 기록 ]**\n" + "\n".join(self.history)

        embed = discord.Embed(
            title=f"🚽 [{self.clicks + 1}/3회차] 쾌변 타일 선택 중!",
            description=desc,
            color=0x9b59b6
        )
        return embed

class PoopLobbyView(discord.ui.View):
    ...
    def build_embed(self, user, u):
        ...
        embed.set_thumbnail(url=get_img_url(f"똥_{p_lvl}단계.png"))
        return embed

    @discord.ui.button(label="💩 똥싸기 시작 (30분 쿨)", style=discord.ButtonStyle.success, row=0)
    async def start_poop(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.user_id:
            await interaction.response.send_message("❌ 본인 대기실입니다.", ephemeral=True)
            return
        ...
        if now - u.get("last_poop_time", 0) < 1800:
            remain = int(1800 - (now - u.get("last_poop_time", 0)))
            await interaction.response.send_message(f"⏳ 아직 장이 차오르지 않았습니다! ({remain // 60}분 {remain % 60}초 후 가능)", ephemeral=True)
            return

        if u.get("poop_count_today", 0) >= 5:
            await interaction.response.send_message("❌ 오늘 일일 똥싸기 횟수(5회)를 모두 사용하셨습니다!", ephemeral=True)
            return

        u["last_poop_time"] = now
        u["poop_count_today"] = u.get("poop_count_today", 0) + 1
        save_data(data)
    ...
    async def upgrade_poop(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if p_lvl >= 5:
            await interaction.response.send_message("✨ 이미 똥싸기 최고 레벨(Lv.5 MAX)에 달성하셨습니다!", ephemeral=True)
            return

        next_spec = POOP_LEVEL_SPECS[p_lvl + 1]
        req_money = next_spec["cost"]
        req_art = next_spec["art_cost"]

        if u["money"] < req_money:
            await interaction.response.send_message(f"❌ 현금이 부족합니다! ({req_money:,}원 필요)", ephemeral=True)
            return
        if u["inventory"].get("똥먹방 비법서", 0) < req_art:
            await interaction.response.send_message(f"❌ [똥먹방 비법서] 유물이 부족합니다! ({req_art}개 필요)", ephemeral=True)
            return

        u["money"] -= req_money
        u["inventory"]["똥먹방 비법서"] -= req_art
        u["poop_level"] += 1
        save_data(data)

        new_embed = self.build_embed(interaction.user, u)
        await interaction.response.edit_message(embed=new_embed, view=self)
        await interaction.followup.send(f"🎉 **[똥싸기 Lv.{u['poop_level']}]** 레벨업 성공! 이제 보상이 {POOP_LEVEL_SPECS[u['poop_level']]['mult']}배 상향됩니다.", ephemeral=True)
...
class StatUpgradeView(discord.ui.View):
    ...
    def build_embed(self, user, u):
        h_lvl = u.get("stat_health_level", 0)
        t_lvl = u.get("stat_tenacity_level", 0)

        h_max = get_max_fatigue(u)
        t_rate = TENACITY_STAT_SPECS.get(t_lvl, {}).get("rate", 0.0)

        h_next = HEALTH_STAT_SPECS.get(h_lvl + 1)
        t_next = TENACITY_STAT_SPECS.get(t_lvl + 1)

        h_str = f"**Lv.{h_lvl}** (최대 피로도: `{h_max}`)"
        if h_next: h_str += f" ➔ 다음 비용: `{h_next['cost']:,}원`"
        else: h_str += " ✨ **[MAX 최고등급]**"

        t_str = f"**Lv.{t_lvl}** (노코스트 확률: `{t_rate}%`)"
        if t_next: t_str += f" ➔ 다음 비용: `{t_next['cost']:,}원`"
        else: t_str += " ✨ **[MAX 최고등급]**"

        embed = discord.Embed(title=f"🏋️‍♂️ {user.display_name}님의 능력치 정비소", color=0x3498db)
        embed.set_thumbnail(url=user.display_avatar.url)
        embed.add_field(name="🔋 체력 (최대 피로도)", value=h_str, inline=False)
        embed.add_field(name="🛡️ 근성 (노코스트 확률)", value=t_str, inline=False)
        embed.add_field(name="💳 보유 현금", value=f"**{u['money']:,}원**", inline=False)
        return embed

    @discord.ui.button(label="🔋 체력 강화", style=discord.ButtonStyle.primary, row=0)
    async def upgrade_health(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.user_id:
        ...
        data = load_data()
        u = get_user_data(data, interaction.user.id)
        curr_lvl = u.get("stat_health_level", 0)

        if curr_lvl >= 10:
            await interaction.response.send_message("✨ 이미 체력 최고 등급(Lv.10)에 달성하셨습니다!", ephemeral=True)
            return

        spec = HEALTH_STAT_SPECS[curr_lvl + 1]
        if u["money"] < spec["cost"]:
            await interaction.response.send_message(f"❌ 현금이 부족합니다! ({spec['cost']:,}원 필요)", ephemeral=True)
            return

        u["money"] -= spec["cost"]
        u["stat_health_level"] += 1
        save_data(data)

        new_embed = self.build_embed(interaction.user, u)
        await interaction.response.edit_message(embed=new_embed, view=self)
        await interaction.followup.send(f"🎉 **[체력 Lv.{u['stat_health_level']}]** 강화 성공! 최대 피로도가 {spec['max_fatigue']}(으)로 확장되었습니다.", ephemeral=True)

    @discord.ui.button(label="🛡️ 근성 강화", style=discord.ButtonStyle.success, row=0)
    async def upgrade_tenacity(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.user_id:
            await interaction.response.send_message("❌ 본인만 조작할 수 있습니다.", ephemeral=True)
            return
        ...
        data = load_data()
        u = get_user_data(data, interaction.user.id)
        curr_lvl = u.get("stat_tenacity_level", 0)

        if curr_lvl >= 3:
            await interaction.response.send_message("✨ 이미 근성 최고 등급(Lv.3)에 달성하셨습니다!", ephemeral=True)
            return

        spec = TENACITY_STAT_SPECS[curr_lvl + 1]
        if u["money"] < spec["cost"]:
            await interaction.response.send_message(f"❌ 현금이 부족합니다! ({spec['cost']:,}원 필요)", ephemeral=True)
            return

        u["money"] -= spec["cost"]
        u["stat_tenacity_level"] += 1
        save_data(data)

        new_embed = self.build_embed(interaction.user, u)
        await interaction.response.edit_message(embed=new_embed, view=self)
        await interaction.followup.send(f"🎉 **[근성 Lv.{u['stat_tenacity_level']}]** 강화 성공! 노코스트 발동 확률이 {spec['rate']}%(으)로 상승했습니다.", ephemeral=True)

# --- 상점 & 경험치 영양제 UI ---
class ExpPotionSizeView(discord.ui.View):
    ...
    async def buy_exp(self, interaction: discord.Interaction, size: str, exp_amount: int, cost: int):
        ...
        u["money"] -= cost
        if self.pot_type == "dance":
            u["dance_count"] += exp_amount
            lvl = update_dance_level(u)
            type_str = "🕺 춤추기"
            emb = get_dance_emblem(lvl)
        else:
            u["fish_count"] += exp_amount
            lvl = update_fish_level(u)
            type_str = "🎣 낚시"
            emb = get_fish_emblem(lvl)

        save_data(data)
        for item in self.children: item.disabled = True
        await interaction.response.edit_message(content=f"✅ **{type_str} 경험치 영양제 [{size}]** 구매 완료!\n• 경험치 **+{exp_amount}** 획득 | 숙련도: {emb} Lv.{lvl}", view=self)

    @discord.ui.button(label="🧪 S (경험치 +15) - 30만 원", style=discord.ButtonStyle.secondary, row=0)
    async def s_btn(self, interaction, button): await self.buy_exp(interaction, "S", 15, 300000)

    @discord.ui.button(label="🧪 M (경험치 +45) - 80만 원", style=discord.ButtonStyle.primary, row=0)
    async def m_btn(self, interaction, button): await self.buy_exp(interaction, "M", 45, 800000)

    @discord.ui.button(label="🧪 L (경험치 +75) - 110만 원", style=discord.ButtonStyle.success, row=0)
    async def l_btn(self, interaction, button): await self.buy_exp(interaction, "L", 75, 1100000)
...
class ShopSelectView(discord.ui.View):
    def __init__(self, user_id):
        ...
        options = [
            ...
            discord.SelectOption(label="🕺 춤 경험치 영양제 (S/M/L)", description="₩ 30만 ~ 110만 원", emoji="🧪", value="exp_dance"),
            discord.SelectOption(label="🎣 낚시 경험치 영양제 (S/M/L)", description="₩ 30만 ~ 110만 원", emoji="🧪", value="exp_fish"),
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        if item_name in ["exp_dance", "exp_fish"]:
            pot_type = "dance" if item_name == "exp_dance" else "fish"
            title_str = "🕺 춤 경험치 영양제" if pot_type == "dance" else "🎣 낚시 경험치 영양제"
            exp_view = ExpPotionSizeView(self.user_id, pot_type)
            await interaction.response.send_message(f"🧪 **[{title_str}]** 구매할 사이즈를 선택하세요:", view=exp_view, ephemeral=True)
            return
...
class BagDynamicSelectView(discord.ui.View):
    def __init__(self, user_id, user_data):
        ...
        if inv.get("핫식스 박스", 0) > 0:
            options.append(discord.SelectOption(label="핫식스 박스 사용", description=f"보유: {inv['핫식스 박스']}개 | 피로도 완충", emoji="⚡", value="use_핫식스 박스"))
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        if val.startswith("use_"):
            ...
            if item == "에너지드링크":
                if u["drink_used_today"] >= 6:
                    await interaction.response.send_message("❌ 하루 최대 6회만 사용 가능합니다.", ephemeral=True)
                    return
                u["inventory"][item] -= 1
                u["drink_used_today"] += 1
                u["fatigue"] = min(max_f, u["fatigue"] + 30)
                msg = f"🥤 에너지드링크 사용 완료! ({u['drink_used_today']}/6회 | 피로도: {u['fatigue']}/{max_f})"
            elif item == "핫식스 박스":
                if u["hot6_used_today"] >= 2:
                    await interaction.response.send_message("❌ 하루 최대 2회만 사용 가능합니다.", ephemeral=True)
                    return
                u["inventory"][item] -= 1
                u["hot6_used_today"] += 1
                u["fatigue"] = max_f
                msg = f"⚡ 핫식스 사용 완료! ({u['hot6_used_today']}/2회) 피로도가 {max_f}(으)로 완충되었습니다!"
            elif item == "유물 랜덤 상자":
                u["inventory"][item] -= 1
...
def build_fish_bag_embed(user, u):
    ...
    embed = discord.Embed(title=f"🐟 {user.display_name}님의 물고기 가방", color=0x3498db)
    lines = []
    total_val = 0
    ...
    for item_key, cnt in inv.items():
        if cnt <= 0: continue
        base_fish = item_key.replace("🌀 혼돈 ", "").replace("🐘 거대 ", "")
        if base_fish in fish_market:
            has_fish = True
            p_info = fish_market[base_fish]
            base_price = p_info["price"] if isinstance(p_info, dict) else p_info

            mult = 1.0
            if "🌀 혼돈" in item_key: mult = 3.0
            elif "🐘 거대" in item_key: mult = 2.0

            unit_price = int(base_price * mult)
            sum_p = unit_price * cnt
            total_val += sum_p
            lines.append(f"• **{item_key}**: `{cnt}마리` (단가: {unit_price:,}원 | 총 `{sum_p:,}원`)")

    if not has_fish:
        lines.append("📦 가방에 보관 중인 물고기가 없습니다.")

    embed.description = "\n".join(lines)
    embed.add_field(name="💰 해산물 총 평가액", value=f"**{total_val:,}원**", inline=False)
    return embed
...
class FishBagView(discord.ui.View):
    def __init__(self, user_id, user_data):
        ...
        for item_key, cnt in user_data["inventory"].items():
            if cnt > 0:
                base_fish = item_key.replace("🌀 혼돈 ", "").replace("🐘 거대 ", "")
                if base_fish in fish_market:
                    options.append(discord.SelectOption(label=f"{item_key} 매도", description=f"보유: {cnt}마리", emoji="🐟", value=item_key))

        if options:
            select = discord.ui.Select(placeholder="🐟 개별 판매할 물고기 선택...", min_values=1, max_values=1, options=options[:25])
            select.callback = self.select_callback
            self.add_item(select)
    ...
    async def select_callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.user_id:
            await interaction.response.send_message("❌ 본인만 조작할 수 있습니다.", ephemeral=True)
            return
        fish_item_key = interaction.data["values"][0]
        modal = FishSellModal(fish_item_key, self.user_id)
        await interaction.response.send_modal(modal)

    @discord.ui.button(label="💰 전체 일괄 매도", style=discord.ButtonStyle.success, row=1)
    async def sell_all_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for item_key in list(inv.keys()):
            cnt = inv.get(item_key, 0)
            if cnt > 0:
                base_fish = item_key.replace("🌀 혼돈 ", "").replace("🐘 거대 ", "")
                if base_fish in fish_market:
                    p_info = fish_market[base_fish]
...
class OddsDashboardView(discord.ui.View):
    ...
    def build_embed(self):
        if self.page == 1:
            ...
            embed = discord.Embed(title="🕺 [2페이지] 춤추기 보상 및 당첨 확률", color=0x9b59b6)
            embed.add_field(name="💰 현금 보상 테이블", value="• 소소한 동전 (25.0% | +500원)\n• 뻣뻣한 춤 (35.0% | +1,000원)\n• 현란한 팝핀 (15.0% | +3,000원)\n• 디스코 댄스 (10.0% | +5,000원)\n• ★대폭발★ 클럽 (7.0% | +10,000원)\n• 브레이크 댄스 (3.0% | +50,000원)\n• ✨[전설의 춤신춤왕] (2.0% | +100,000원)", inline=False)
            embed.add_field(name="🎁 특수 아이템 보상", value="• ✨ 바닥 강화석 주입 (1.0% | 강화석 1개)\n• 🏛️ 고대 유물 발굴 (2.0% | 유물 1개 랜덤)", inline=False)
            embed.set_footer(text="💡 근성 스탯 보유 시 피로도 0 소모 확률이 적용됩니다.")
        elif self.page == 3:
            embed = discord.Embed(title="🎣 [3페이지] 낚싯대별 확률 및 변이 안내", color=0x2ecc71)
            for r_id, r_info in ROD_DATA.items():
                trash, norm, rare, leg, box = r_info["rates"]
                embed.add_field(
                    name=f"🎣 {r_info['name']} (피로도 소모 {r_info['fatigue']})",
                    value=f"• 🗑️ 쓰레기: **{trash}%** | 🐟 일반: **{norm}%** | 🐠 희귀: **{rare}%**\n• 👑 전설: **{leg}%** | 🎁 보물상자: **{box}%**",
                    inline=False
                )
            embed.add_field(name="🌟 물고기 변이 확률 (일반 물고기 낚을 시 독립 적용)", value="• 🌀 **[혼돈]** (1.0% 확률 | 가치 3.0배)\n• 🐘 **[거대]** (1.5% 확률 | 가치 2.0배)", inline=False)
        else:
            embed = discord.Embed(title="💣 [4페이지] 박스 단계별 가치 및 확률 안내", color=0xe74c3c)
            lines = [
...
class RPSDuelView(discord.ui.View):
    ...
    @discord.ui.button(label="⚔️ 대결 신청", style=discord.ButtonStyle.danger, row=0)
    async def join_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if c_data["money"] < self.bet_amount:
            await interaction.response.send_message(f"❌ 베팅금이 부족합니다! ({self.bet_amount:,}원 필요)", ephemeral=True)
            return
        ...
        embed = discord.Embed(
            ...
            description=f"👑 **{self.host_user.mention}** vs ⚔️ **{self.challenger.mention}**\n\n두 분 모두 아래 버튼을 눌러 제출하세요!",
            color=0xf1c40f
        )
        play_view = RPSChoiceView(self)
...
class RPSChoiceView(discord.ui.View):
    ...
    async def make_choice(self, interaction: discord.Interaction, choice: str):
        ...
        if uid not in [self.main_view.host_user.id, self.main_view.challenger.id]:
            await interaction.response.send_message("❌ 대결 당사자만 선택할 수 있습니다.", ephemeral=True)
            return

        if uid == self.main_view.host_user.id:
            if self.main_view.host_choice:
                await interaction.response.send_message("❌ 이미 선택하셨습니다.", ephemeral=True)
                return
            self.main_view.host_choice = choice
        else:
            if self.main_view.challenger_choice:
                await interaction.response.send_message("❌ 이미 선택하셨습니다.", ephemeral=True)
                return
            self.main_view.challenger_choice = choice

        await interaction.response.send_message(f"✅ [{choice}] 선택 완료!", ephemeral=True)

        if self.main_view.host_choice and self.main_view.challenger_choice:
            for item in self.children: item.disabled = True

            hc = self.main_view.host_choice
            cc = self.main_view.challenger_choice

            data = load_data()
            h_data = get_user_data(data, self.main_view.host_user.id)
            c_data = get_user_data(data, self.main_view.challenger.id)
            pot = self.main_view.bet_amount * 2

            if hc == cc:
                h_data["money"] += self.main_view.bet_amount
                c_data["money"] += self.main_view.bet_amount
                res_str = "🤝 **무승부!** (베팅금 100% 환불)"
                color = 0x95a5a6
            elif (hc == "✌️ 가위" and cc == "🖐️ 보") or (hc == "✊ 바위" and cc == "✌️ 가위") or (hc == "🖐️ 보" and cc == "✊ 바위"):
                h_data["money"] += pot
                res_str = f"🎉 **{self.main_view.host_user.mention} 승리!** (+{pot:,}원)"
                color = 0x2ecc71
            else:
                c_data["money"] += pot
                res_str = f"🎉 **{self.main_view.challenger.mention} 승리!** (+{pot:,}원)"
                color = 0x2ecc71

            save_data(data)
            res_embed = discord.Embed(title="✌️ 가위바위보 결과 발표!", color=color)
            res_embed.add_field(name=f"👑 {self.main_view.host_user.display_name}", value=f"**{hc}**", inline=True)
            res_embed.add_field(name=f"⚔️ {self.main_view.challenger.display_name}", value=f"**{cc}**", inline=True)
            res_embed.add_field(name="🏆 최종 결과", value=res_str, inline=False)
            await interaction.message.edit(embed=res_embed, view=None)

    @discord.ui.button(label="✌️ 가위", style=discord.ButtonStyle.primary)
    async def s_btn(self, interaction, button): await self.make_choice(interaction, "✌️ 가위")

    @discord.ui.button(label="✊ 바위", style=discord.ButtonStyle.danger)
    async def r_btn(self, interaction, button): await self.make_choice(interaction, "✊ 바위")

    @discord.ui.button(label="🖐️ 보", style=discord.ButtonStyle.success)
    async def p_btn(self, interaction, button): await self.make_choice(interaction, "🖐️ 보")
...
class CircuitPreView(discord.ui.View):
    ...
    async def cancel_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.user_id:
            ...
            return
        for item in self.children: item.disabled = True
        await interaction.response.edit_message(content="🛑 도전을 취소했습니다.", view=self)
...
class CircuitInRaceEventView(discord.ui.View):
    ...
    @discord.ui.button(label="🔥 오버클럭 부스터!", style=discord.ButtonStyle.danger)
...
async def run_circuit_race(interaction, floor):
    ...
    lvl_diff = max(0, rec_lvl - car_lvl)
    speed_penalty = max(0.20, 1.0 - (lvl_diff * 0.08))

    while not winner:
        await asyncio.sleep(1.2)

        raw_p_speed = int(8 + (car_lvl * 1.8) + random.randint(-2, 4))
        p_speed = max(2, int(raw_p_speed * speed_penalty))
        ...
        if p_dist >= 50 and not event_triggered and p_dist < 100 and b_dist < 100:
            ...
            ev_embed = discord.Embed(title="⚡ [50m 코너 지점] 전술 선택!", description="승부를 가를 코너 구간입니다!", color=0xf1c40f)
            await msg.edit(embed=ev_embed, view=event_view)
            await asyncio.sleep(4)

            oc_success_rate = max(0.10, 0.50 - (lvl_diff * 0.05))

            if event_view.choice == "overclock":
                if random.random() < oc_success_rate:
                    p_dist = min(100, p_dist + 35)
                    ev_msg = "🔥 **오버클럭 대성공!!** 부스터를 터뜨려 +35m 치고 나갑니다!"
                else:
                    p_dist = max(0, p_dist - 10)
                    ev_msg = "💥 **벽 충돌!!** 감속되었습니다. (-10m)"
            else:
...
def build_bag_embed(user, u):
    ...
    if len(lines) == 1: lines.append("📦 소지한 소모품/장비가 없습니다. (물고기는 /물고기가방 참조)")
...
class CatchFishButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if rand < box_r:
            ...
            if rand < (box_r + leg_r):
                fish = "황금 잉어"
                rank_str, embed_color = "👑 [전설]", 0xf1c40f
            elif rand < (box_r + leg_r + rare_r):
                fish = random.choice(["참다랑어", "돗돔"])
                rank_str, embed_color = "🐠 [희귀]", 0x3498db
                ...
                u["inventory"][trash] = u["inventory"].get(trash, 0) + 1
                res_title, res_desc, embed_color, img_file = f"🗑️ {trash}", f"{trash}을(를) 낚았습니다...", 0x95a5a6, "낚시_쓰레기.png"

            if not is_trash:
                mut_rand = random.uniform(0, 100)
                file_fish_name = "황금잉어" if fish == "황금 잉어" else fish

                if mut_rand < 1.0:
                    mut_prefix = "🌀 혼돈"
                    item_key = f"🌀 혼돈 {fish}"
                    img_file = f"혼돈_{file_fish_name}.png"
                    res_desc = f"**{mut_prefix} {fish}**을(를) 낚았습니다! (x3.0 가치!)"
                elif mut_rand < 2.5:
                    mut_prefix = "🐘 거대"
                    item_key = f"🐘 거대 {fish}"
                    img_file = f"거대_{file_fish_name}.png"
                    res_desc = f"**{mut_prefix} {fish}**을(를) 낚았습니다! (x2.0 가치!)"
                else:
                    mut_prefix = ""
                    item_key = fish
                    img_file = f"낚시_{file_fish_name}.png"
                    res_desc = f"{fish}을(를) 낚아 가방에 보관했습니다!"

                u["inventory"][item_key] = u["inventory"].get(item_key, 0) + 1
                res_title = f"{rank_str} {mut_prefix} {fish}".strip()
        ...
        max_f = get_max_fatigue(u)
        full_desc = f"{res_desc}\n\n{fish_emb} **낚시** Lv.{u['fish_level']} ({u['fish_count']}회)"

        res_embed = discord.Embed(title=res_title, description=full_desc, color=embed_color)
        res_embed.set_image(url=get_img_url(img_file))
        res_embed.set_footer(text=f"⚡ 남은 피로도: {u['fatigue']}/{max_f}")
...
async def my_info(interaction: discord.Interaction):
    ...
    desc = (
        ...
        f"• **피로도:** {get_fatigue_bar(u['fatigue'], max_f)}\n"
        f"• **장비:** {get_car_name(u['car_level'])} | {eq_rod_name}\n"
        f"• **똥싸기 등급:** Lv.{u.get('poop_level', 1)}\n\n"
        f"{dance_emb} **춤추기** Lv.{u['dance_level']} ({u['dance_count']}회)\n"
        f"{fish_emb} **낚시** Lv.{u['fish_level']} ({u['fish_count']}회)\n\n"
...
async def poop_command(interaction: discord.Interaction):
    data = load_data()
    u = get_user_data(data, interaction.user.id)
    view = PoopLobbyView(interaction.user.id)
...
async def stat_upgrade(interaction: discord.Interaction):
    data = load_data()
    u = get_user_data(data, interaction.user.id)
    view = StatUpgradeView(interaction.user.id)
    embed = view.build_embed(interaction.user, u)
...
async def fish_bag(interaction: discord.Interaction):
    data = load_data()
    u = get_user_data(data, interaction.user.id)
    embed = build_fish_bag_embed(interaction.user, u)
...
@app_commands.choices(층=[
    ...
    app_commands.Choice(name="2층: 수도권 외곽순환 (추천 8강)", value=2),
    app_commands.Choice(name="3층: 태백 레이스웨이 (추천 13강)", value=3),
    app_commands.Choice(name="4층: 영암 F1 서킷 (추천 18강)", value=4),
    app_commands.Choice(name="5층: 뉘르부르크링 (추천 23강)", value=5),
    app_commands.Choice(name="6층: 아우토반 무제한 (추천 28강)", value=6)
])
async def circuit_dungeon(interaction: discord.Interaction, 층: int):
    ...
    p_avg = max(2, int((8 + (car_lvl * 1.8) + 1) * speed_penalty))
    b_avg = d_info["boss_speed"] + 1
    est_win_rate = min(99, max(1, int((p_avg / (p_avg + b_avg)) * 100)))

    max_f = get_max_fatigue(u)
    penalty_warn = f"\n⚠️ **권장 강수({rec_lvl}강) 미달 페널티:** 속도가 `{int((1 - speed_penalty)*100)}%` 감소합니다!" if lvl_diff > 0 else ""
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"• **보스:** {d_info['boss']}\n"
            f"• **예상 승률:** 🔥 **{est_win_rate}%**{penalty_warn}\n"
            f"• **필요 피로도:** ⚡ 20 (보유: {u['fatigue']}/{max_f})\n\n"
            f"도전하시겠습니까?"
        ),
        color=0x3498db
    )
...
async def dance(interaction: discord.Interaction):
    ...
    actual_cost = 0 if tenacity_triggered else fatigue_cost

    if u["fatigue"] < actual_cost:
        await interaction.response.send_message("❌ 피로도가 부족합니다!", ephemeral=True)
        return

    u["fatigue"] -= actual_cost
    ...
    max_f = get_max_fatigue(u)
    tenacity_msg = "\n✨ **[근성 발동!]** 피로도를 소모하지 않았습니다!" if tenacity_triggered else ""
    full_desc = f"{msg}\n• 보상: **{reward_str}**{tenacity_msg}\n\n{dance_emb} **춤추기** Lv.{u['dance_level']} ({u['dance_count']}회)"

    embed = discord.Embed(title="🕺 춤추기 완료", description=full_desc, color=embed_color)
...
async def fishing(interaction: discord.Interaction):
    ...
    if u["fatigue"] < actual_cost:
        await interaction.response.send_message(f"❌ 피로도가 부족합니다! ({actual_cost} 필요)", ephemeral=True)
        return
    ...
    cost_msg = "✨ [근성 발동!] 피로도 0 소모" if tenacity_triggered else f"피로도 -{actual_cost}"
    await interaction.response.send_message(f"🎣 **[{rod_info['name']}]**(으)로 찌를 멀리 던졌습니다... ({cost_msg})")
...
async def rps_duel(interaction: discord.Interaction, 베팅금: int):
    ...
    embed = discord.Embed(
        title="✌️ 1v1 가위바위보 대결 신청",
        description=f"**{interaction.user.display_name}**님이 **{베팅금:,}원** 가위바위보 대결을 신청하셨습니다!",
        color=0xf1c40f
    )
    view = RPSDuelView(interaction.user, 베팅금)
    await interaction.response.send_message(embed=embed, view=view)
